    ...
    public static ArrayList<double?> decodeAndPlot(ProjectionNumbers projectionNumbers, bool isGust, FileStorage fileStorage) {
        ...
        foreach (var windBarb in arrWb) {
            ...
            if (length > 4) {
                ...
                tmpCoords = NexradLevel3Common.computeMercatorNumbersFromEc(ec, projectionNumbers);
                stormList.add(tmpCoords[0]);
                stormList.add(tmpCoords[1]);
                ...
                if (above50) {
                    ...
                    stormList.add_all(NexradLevel3Common.drawLine(ec, ecc, projectionNumbers, degree2 - arrowBend * 2.0, startLength + arrowLength * nmScaleFactor));
                    ...
                    stormList.add_all(NexradLevel3Common.drawLine(ec, ecc, projectionNumbers, degree2 - 90.0, startLength + 0.80 * arrowLength * nmScaleFactor));
                    // connecting line parallel to main barb
                    ec = ecc.calculateEndingGlobalCoordinates(end, degree2, barbOffset + startLength + index * arrowSpacing * nmScaleFactor * barbLengthScaleFactor);
                    stormList.add_all(NexradLevel3Common.drawLine(ec, ecc, projectionNumbers, degree2 - 180.0, startLength + 0.5 * arrowLength * nmScaleFactor));
                    index += 1;
                }
                ...
                if (halfBarb) {
                    ...
                    stormList.add_all(NexradLevel3Common.drawLine(ec, ecc, projectionNumbers, degree2 - arrowBend * 2.0, startLength + arrowLength / 2.0 * nmScaleFactor));
                }
            }
        }
        return stormList;
