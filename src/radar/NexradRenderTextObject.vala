...
public class NexradRenderTextObject {
    ...
    public NexradRenderTextObject(int numPanes, NexradState nexradState, FileStorage fileStorage) {
    ...
    void addTextLabelsCitiesExtended() {
        if (RadarPreferences.cities) {
            ...
            if (nexradState.zoom > cityMinZoom) {
                var cityExtLength = CitiesExtended.cities.size;
                foreach (var index in range(cityExtLength)) {
                    if (nexradState.cities.size <= maxCitiesPerGlview) {
                        checkAndDrawText(nexradState.cities, CitiesExtended.cities[index].latitude, CitiesExtended.cities[index].longitude, CitiesExtended.cities[index].name, true);
                    }
                }
            }
        }
    ...
    void checkAndDrawText(ArrayList<TextViewMetal> tvList, double lat, double lon, string text, bool checkBounds) {
        var latLon = Projection.computeMercatorNumbers(lat, -1.0 * lon, nexradState.getPn());
        var xPos = latLon[0];
        var yPos = latLon[1];
    ...
    void initializeTextLabelsCitiesExtended() {
        if (numPanes == 1 && RadarPreferences.cities && !CitiesExtended.initialized) {
            CitiesExtended.create();
        }
    }

    void initializeTextLabelsCountyLabels() {
        if (RadarPreferences.countyLabels && !CountyLabels.initialized) {
            CountyLabels.create();
        }
    }
    ...
    void addTextLabelsCountyLabels() {
        if (RadarPreferences.countyLabels) {
            ...
            if (nexradState.zoom > countyMinZoom) {
                foreach (var index in range(CountyLabels.names.size)) {
                    checkAndDrawText(nexradState.countyLabels, CountyLabels.location[index].lat(), CountyLabels.location[index].lon(), CountyLabels.names[index], true);
                }
            }
        }
    }
    ...
    public void addWpcPressureCenters() {
        if (RadarPreferences.wpcFronts) {
            ...
            if (nexradState.zoom < nexradState.zoomToHideMiscFeatures) {
                foreach (var p in WpcFronts.pressureCenters) {
