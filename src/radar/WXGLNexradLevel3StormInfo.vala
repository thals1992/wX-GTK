...
class WXGLNexradLevel3StormInfo {
    ...
    public static void decode(ProjectionNumbers projectionNumbers, FileStorage fileStorage) {
        ...
        if (dis != "") {
            ...
            var motion = UtilityString.parseColumn(dis, "MVT(.*?)V");
            var posnStr = "";
            ...
            if ((posnNumbers.size == motNumbers.size) && posnNumbers.size > 1) {
                foreach (var index in range3(0, posnNumbers.size - 2, 2)) {
                    ...
                    var coord = WXGLNexradLevel3Common.computeMercatorNumbersFromEc(ec, projectionNumbers);
    ...
    static ArrayList<double?> drawTickMarks(
        ...
        var coordinates = WXGLNexradLevel3Common.computeMercatorNumbersFromEc(ec, pn);
