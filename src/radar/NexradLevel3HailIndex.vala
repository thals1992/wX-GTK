...
class NexradLevel3HailIndex {
    ...
    public static void decode(ProjectionNumbers projectionNumbers, FileStorage fileStorage) {
        var data1 = NexradLevel3TextProduct.download("HI", projectionNumbers.getRadarSite());
        var posn = UtilityString.parseColumn(data1, "AZ/RAN(.*?)V");
        var hailPercent = UtilityString.parseColumn(data1, "POSH/POH(.*?)V");
        var hailSize = UtilityString.parseColumn(data1, "MAX HAIL SIZE(.*?)V");
        var posnStr = "";
