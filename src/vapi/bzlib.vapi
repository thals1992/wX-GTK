...
namespace BZLib {
	...
	[CCode (cname = "BZ2_bzBuffToBuffDecompress")]
    public int BuffToBuffDecompress ([CCode(array_length = false)] uint8[] destBuf, ref int readSize, [CCode(array_length = false)] uint8[] srcBuf, int inSize, int small, int verbosity);
    // The compressed data is sent invia srcBuf
    // In the caller destBuf is sized to a specific value, ie "readSize". The BZ function then writes the totalbytes uncompressed to this same value
    // Example usage:
        // int size = 1000000;
        // uint8[] oBuff = new uint8[size];
        // BZLib.BuffToBuffDecompress(oBuff, ref size, compressedData, compressedData.length, 1, 0);
        // print("BZIP bytes read " + ": " + too.String(size) + "\n");
        // return oBuff[0:size];
}
