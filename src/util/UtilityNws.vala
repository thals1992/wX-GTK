...
class UtilityNws {
    ...
    const int numHeight = 20;
    const double fontSize = 16.0;
    const double rectSize = 20.0;
    const int spaceFromEdge = 4;
    ...
    public static Gdk.Pixbuf getIcon(string url) {
        ...
        if (UtilityList.inIt(fileName + ".png", UtilityNwsIcon.iconMap)) {
            try {
                // Gdk.Pixbuf pix = new Gdk.Pixbuf.from_file(GlobalVariables.imageDir + fileName + ".png");
                var pix = new Gdk.Pixbuf.from_resource("/" + GlobalVariables.imageDir + fileName + ".png");
                pix = pix.scale_simple(dimensions, dimensions, Gdk.InterpType.BILINEAR);
                return pix;
            } catch(Error e) {
                print(e.message + "\n");
                return new Gdk.Pixbuf(Gdk.Colorspace.RGB, true, 8, dimensions, dimensions);
    ...
    public static Gdk.Pixbuf getDualBitmapWithNumbers(string iconLeftString, string iconRightString) {
        ...
        if (leftTokens.length > 1) {
            leftNumber = leftTokens[1];
        }
        var rightNumber = "";
        if (rightTokens.length > 1) {
            rightNumber = rightTokens[1];
        }
        ...
        try {
            ...
            // var surf1 = new Cairo.ImageSurface.from_png(GlobalVariables.imageDir + leftWeatherCondition + ".png");
            // var surf2 = new Cairo.ImageSurface.from_png(GlobalVariables.imageDir + rightWeatherCondition + ".png");

            var img = new Cairo.ImageSurface(Cairo.Format.ARGB32, dimensionInterim, dimensionInterim);
            var ctx = new Cairo.Context(img);
            ctx.set_source_rgba(1.0, 1.0, 1.0, 1.0);
            ctx.paint();

            // ctx.set_source_surface(surf1, 0, 0);
            Gdk.cairo_set_source_pixbuf(ctx, surf1, 0.0, 0.0);
            ctx.paint();

            // ctx.set_source_surface(surf2, dimensionInterim / 2.0, 0.0);
            Gdk.cairo_set_source_pixbuf(ctx, surf2, dimensionInterim / 2.0 + padding, 0.0);
            ctx.paint();

            //  if (leftNumber != "" || rightNumber != "") {
            //      ctx.set_source_rgba(255, 255, 255, 0.75);
            //      ctx.rectangle(0, dimensionInterim - rectSize, dimensionInterim, rectSize);
            //      ctx.fill();
            //  }

            if (leftNumber != "" && leftNumber != "0" ) {
                ctx.set_source_rgb(gray, gray, gray);
                ctx.rectangle(0, dimensionInterim - rectSize, (int)(dimensionInterim / 2.0) - padding, rectSize);
                ctx.fill();

                ctx.set_font_size(fontSize);
                ctx.move_to(spaceFromEdge, dimensionInterim - spaceFromEdge);
                ctx.set_source_rgb(0.15, 0.38, 0.54);
                ctx.show_text(leftNumber + "%");
            }

            if (rightNumber != "" && rightNumber != "0") {
                ctx.set_source_rgb(gray, gray, gray);
                ctx.rectangle((int)(dimensionInterim / 2.0) + padding, dimensionInterim - rectSize, (int)(dimensionInterim / 2.0) - padding, rectSize);
                ctx.fill();

                ctx.set_font_size(fontSize);
                ctx.move_to((dimensionInterim) / 2.0 + padding, dimensionInterim - spaceFromEdge);
                ctx.set_source_rgb(0.15, 0.38, 0.54);
                ctx.show_text(rightNumber + "%");
            }
            var pix = Gdk.pixbuf_get_from_surface(img, 0, 0, dimensionInterim, dimensionInterim);
            pix = pix.scale_simple(dimensions, dimensions, Gdk.InterpType.BILINEAR);
            return pix;
        } catch (Error e) {
            print(e.message + "\n");
