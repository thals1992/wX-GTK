...
class PolygonWatch {
    ...
    public static HashMap<PolygonType, PolygonWatch> byType;
    ...
    public PolygonWatch(PolygonType type) {
        this.type = type;
        ...
        if (timer.isRefreshNeeded()) {
            ...
            if (type == PolygonType.Mpd) {
                ...
                foreach (var number in numbers) {
                    var text = DownloadText.byProduct("WPCMPD" + number);
                ...
                foreach (var number in numbers) {
                    ...
                    var text = DownloadText.byProduct("SPCMCD" + numberModified);
                ...
                latLonList.setValue(latLonString);
                numberList.setValue(numberListString);
                PolygonWatch.byType[PolygonType.WatchTornado].latLonList.setValue(latLonTorString);
    ...
    public static void load() {
        if (byType == null) {
            byType = new HashMap<PolygonType, PolygonWatch>();
        ...
        foreach (var data in polygonList) {
            if (!byType.keys.contains(data)) {
                byType[data] = new PolygonWatch(data);
            } else {
                byType[data].update();
            }
        }
        byType[PolygonType.WatchTornado].isEnabled = byType[PolygonType.Watch].isEnabled;
    }
}
