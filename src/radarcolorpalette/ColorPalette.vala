...
class ColorPalette {
    ...
    public static HashMap<int, ColorPalette> colorMap;
    ...
    public ColorPalette(int colormapCode) {
    ...
    public void putBytesFromLine(ColorPaletteLine objectColorPaletteLine) {
    ...
    static void generate(int productCode, string code) {
        ColorPalette objectColorPalette = ColorPalette.colorMap[productCode];
        objectColorPalette.position(0);
        var objectColorPaletteLines = new ArrayList<ColorPaletteLine>();
        var scale = 2;
        var lowerEnd = -32;
        var prodOffset = 0.0;
        var prodScale = 1.0;
        ...
        foreach (var line in text.split("\n")) {
            if (line.contains("olor") && !line.contains("#")) {
                ...
                if (items.length > 4) {
                    ...
                    blue = items[4];
                    if (priorLineHas6) {
                        objectColorPaletteLines.add(new ColorPaletteLine.withDbzNoList((Too.Double(items[1]) * prodScale + prodOffset - 1), red, green, blue));
                        objectColorPaletteLines.add(new ColorPaletteLine.withDbzNoList((Too.Double(items[1]) * prodScale + prodOffset - 1), red, green, blue));
                        priorLineHas6 = false;
                    } else {
                        objectColorPaletteLines.add(new ColorPaletteLine.withDbzNoList((Too.Double(items[1]) * prodScale + prodOffset - 1), red, green, blue));
    ...
    public static void loadColorMap(int productCode) {
        ...
        var map = ColorPalette.radarColorPalette[productCode];
    ...
    static void generate4bitGeneric(int radarColorPaletteCode) {
        ...
        foreach (var line in text.split("\n")) {
            if (line.contains(",")) {
                var objectColorPaletteLines = new ColorPaletteLine.fourBit(line.split(","));
                ColorPalette.colorMap[radarColorPaletteCode].putBytesFromLine(objectColorPaletteLines);
            }
        }
    }
    ...
    static void loadColorMap165() {
        ...
        foreach (var data in text.split("\n")) {
            if (data.contains("olor") && !data.contains("#")) {
                ...
                if (items.length > 4) {
                    objectColorPaletteLines.add(new ColorPaletteLine(UtilityList.wrap(items)));
                }
            }
        }
        const int diff = 10;
        foreach (var i in range(objectColorPaletteLines.size)) {
            range(diff).foreach((unused) => {
                ColorPalette.colorMap[radarColorPaletteCode].putBytesFromLine(objectColorPaletteLines[i]);
                return true;
            });
        }
    }
